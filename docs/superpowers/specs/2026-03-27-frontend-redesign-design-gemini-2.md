...
- Robust rendering across macOS/Safari and Windows/Edge
...
- **Font:** HarmonyOS Sans (self-hosted woff2) with optimized Windows ClearType fallback
...
- **Sidebar:** Desktop = frosted glass collapsible sider (with robust solid fallback for Edge hardware acceleration disabled); Tablet = icon-only rail; Phone = bottom tab bar
...
:root {
  ...
  /* Sidebar material (Optimized for Windows Edge fallback) */
  --sidebar-bg-solid: rgba(255, 255, 255, 1);
  /* 0.85 opacity ensures text remains readable if backdrop-filter fails on Windows without hardware acceleration */
...
html.dark {
  ...
  /* Sidebar material (Optimized for Windows Edge fallback) */
  --sidebar-bg-solid: rgba(28, 28, 30, 1);
  /* 0.85 opacity ensures text remains readable if backdrop-filter fails */
...
### 1.4 Base Resets & Safe Area (Windows Optimized)
...
body {
  ...
  /* Added Microsoft YaHei for improved Windows ClearType rendering on standard DPI */
  font-family: 'HarmonyOS Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Microsoft YaHei', 'Vazirmatn', Roboto, sans-serif;
  font-size: 14px;
  line-height: 1.5;
...
/* Modern Scrollbars (Native Win 11 & Firefox) */
* {
  scrollbar-width: thin;
  scrollbar-color: var(--color-separator-opaque) transparent;
}

/* Fallback for older Webkit (Win 10 / Old Chrome) */
...
2. Visual check in Chrome/Edge:
   - **Desktop (>1024px):** Sidebar expands/collapses, frosted glass visible. Check on Windows with hardware acceleration turned off to ensure text remains readable.
